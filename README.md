...
### API adresi ortamdan gelir (koda gömülü DEĞİL)

Backend adresi artık kaynak kodda sabit **değildir** (eskiden
`http://34.158.136.9/...` gömülüydü → sunucu taşınınca kod değişmesi gerekiyor,
üstelik düz HTTP). Adres `src/environments/` altından gelir:

| Ortam | `apiBase` | Nasıl |
|-------|-----------|-------|
| **Üretim** | `/emre/2048/api` | same-origin göreli yol → sayfa HTTPS ise API de **HTTPS** (karışık içerik yok, sabit IP/protokol yok) |
| **Geliştirme** | `/api` → `127.0.0.1:8092` | `proxy.conf.json` yerel sunucuya yönlendirir → **üretim sunucusuna bağlanılmaz** |

Yerel geliştirme (iki terminal):

```bash
python server/app.py       # yerel API (127.0.0.1:8092), geçici/yerel DB
npx ng serve               # http://localhost:4200 — /api → localhost:8092 (proxy)
```

### HTTPS (nginx + Let's Encrypt)

Parola ve oturum token'ı **HTTPS zorunludur** (düz HTTP'de aynı Wi-Fi'daki biri
parolayı okuyabilir; 600.000 iterasyonlu hash açık ağda gidince işe yaramaz).
Örnek nginx yapılandırması: [`deploy/nginx-game2048.conf`](deploy/nginx-game2048.conf)
— HTTP→HTTPS kalıcı yönlendirme, HSTS, same-origin API proxy'si içerir.

Kurulum (sunucuda, bir kez — `<ALANADI>` gerçek alan adı):

```bash
# 1) Alan adını sunucunun IP'sine yönlendir (DNS A kaydı).
# 2) nginx conf'u yerleştir (deploy/nginx-game2048.conf içindeki <ALANADI>'ı düzenle)
sudo cp deploy/nginx-game2048.conf /etc/nginx/sites-available/game2048
sudo ln -sf /etc/nginx/sites-available/game2048 /etc/nginx/sites-enabled/
sudo mkdir -p /var/www/certbot

# 3) Let's Encrypt sertifikası al (nginx eklentisi conf'u otomatik günceller)
sudo certbot --nginx -d <ALANADI> --redirect --hsts

sudo nginx -t && sudo systemctl reload nginx
```

**Otomatik yenileme:** certbot kurulumu bir systemd zamanlayıcısı (`certbot.timer`)
ekler; günde iki kez çalışıp süresi dolmaya yakın sertifikayı yeniler ve nginx'i
yeniden yükler. Doğrulama ve kuru prova:

```bash
systemctl list-timers | grep certbot     # zamanlayıcı etkin mi
sudo certbot renew --dry-run              # yenilemenin çalıştığını kanıtla
```

> **Not (bu depoda yapılamayan kısım):** alan adı satın alma, DNS ve sunucuda
> certbot/nginx çalıştırma bu repodan yapılamaz (alan adı + TLS erişimi gerekir).
> Kod/yapılandırma tarafı (ortam-tabanlı API adresi, yerel dev, HSTS'li nginx
> conf, otomatik yenileme adımları) hazırdır; yukarıdaki komutlar sunucuda bir
> kez çalıştırılınca site ve API tümüyle HTTPS olur.

### Oturum ve parola güvenliği

- **Token ömrü:** 90 gün → **30 gün** (atıl süre) indirildi ve **kayan pencereye**
  geçirildi: jeton her kullanımda yenilenir (aktif kullanıcı çıkış yapmaz), ama
  30 gün hiç kullanılmayan oturum ölür → sızan bir jetonun ömrü sınırlı. (Tam
  yenileme-jetonu rotasyonu ileri bir iyileştirme olarak not edildi.)
- **Parola değiştirme bildirimi (plan):** Parolalar bir süre düz HTTP üzerinden
  gitmiş olabilir. HTTPS'e geçiş dağıtımıyla birlikte, girişte tek seferlik bir
  bilgilendirme gösterilmeli: *"Güvenlik güncellemesi yaptık; parolanı
  değiştirmeni öneririz."* Uygulama planı: (1) HTTPS dağıtımı yapılır, (2) 30
  günlük token daraltması zaten çoğu kullanıcıyı yeniden girişe yönlendirir,
  (3) profil/ayarlarda "Parolayı değiştir" akışı öne çıkarılır, (4) bir defalık
  banner i18n anahtarıyla eklenir. (Banner/akış bu pakette değil; kritik olan
  HTTPS + adres taşıma önce yapıldı.)
